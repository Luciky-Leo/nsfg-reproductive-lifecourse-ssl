...
Analysis scripts, split definitions, source-data tables, model metadata, and figure assets are available in the public GitHub repository \url{{https://github.com/Luciky-Leo/nsfg-reproductive-lifecourse-ssl}} and the archived Zenodo release \url{{https://doi.org/10.5281/zenodo.20692052}}. Raw NSFG individual-level public-use records are not redistributed; the scripts point to CDC/NCHS public-use data portals and document the processing workflow. Project code is released under the MIT License, and citation metadata are provided in \texttt{{CITATION.cff}}. The repository includes the feature-audit files needed to identify primary encoder inputs, candidate retained variables, excluded sparse variables, and endpoint-direct exclusion rules.
